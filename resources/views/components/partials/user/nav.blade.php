<nav class="bg-white border-gray-200 dark:bg-purple-900 sticky top-0 z-99999">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <a href="https://flowbite.com/" class="flex items-center space-x-3 rtl:space-x-reverse">
            <img :src="darkMode ? '{{ asset('assets/images/logo/logo-light.png') }}' :
                '{{ asset('assets/images/logo/logo-dark.png') }}'" width="40px" height="auto" class="duration-300" alt="Logo" />
        ...
        <div class="hidden w-full md:flex md:flex-row md:w-auto" id="navbar-default" x-data="{ selected: location.pathname }">
            <ul class="font-medium flex flex-col md:items-center p-4 md:p-0 mt-4 border  rounded-lg  md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 ">
                <li>
                    <a href="{{ route('user.home') }}" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent" aria-current="page" :class="{ '!text-blue-700 !dark:text-gray-400': (selected.includes('home')) }">Home</a>
                </li>
                <li>
                    <a href="{{ route('user.penduduk') }}" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent" :class="{ '!text-blue-700 !dark:text-gray-400': (selected.includes('penduduk')) }">Penduduk</a>
                </li>
                <li>
                    <a href="{{ route('user.berita') }}" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent" :class="{ '!text-blue-700 !dark:text-gray-400': (selected.includes('berita')) }">Berita</a>
                </li>
                <li>
                    <a href="{{ route('user.agenda') }}" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent" :class="{ '!text-blue-700 !dark:text-gray-400': (selected.includes('agenda')) }">Agenda</a>
                </li>
                <li>
                    <a href="{{ route('user.umkm') }}" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent" :class="{ '!text-blue-700 !dark:text-gray-400': (selected.includes('umkm')) }">UMKM</a>
                </li>
                <li>
                    <a href="{{ route('user.inventaris') }}" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent" :class="{ '!text-blue-700 !dark:text-gray-400': (selected.includes('inventaris')) }">inventaris</a>
                </li>
                <li>
  ...
  -->
                ...
                <li>
                    @guest
                    <div class="py-1">
                        <a href="{{ route('login') }}" class="block px-5 py-2 mr-5 bg-indigo-950 rounded-full text-white">Login</a>
                    </div>
                    @endguest
                </li>
            </ul>
            ...
            <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
                <a class="flex items-center gap-4" href="#" @click.prevent="dropdownOpen = ! dropdownOpen">
                    ...
                    <svg :class="dropdownOpen && 'rotate-180'" class="hidden fill-current sm:block dark:text-white" width="12" height="8" viewBox="0 0 12 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <!-- Dropdown Start -->
                <div x-show="dropdownOpen" class="absolute right-0 mt-4 flex w-62.5 flex-col rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark ">
                    <ul class="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark">
                        <li {{ auth()->user()->level->nama_level == 'Penduduk' ? 'hidden' : '' }}>
                            <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base">
                        ...
                        <li>
                            <a href="{{ route('user.berita.dashboard') }}" class="flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base">
                        ...
                        <li>
                            <a href="{{ route('user.bansos') }}" class="flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base">
                                <svg width="22" height="22" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512">
                                    <!--!Font Awesome Free 6.5.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                    ...
                    <form action="{{ route('logout') }}" method="post">
                        @csrf
                        <button class="flex items-center gap-3.5 px-6 py-4 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base w-100">
