    ...
    <section class="row max-w-6xl mx-auto font-sans py-12">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                <a href="#">
                    <img class="rounded-t-lg h-70 bg-purple-400 " src="{{ $item->getImage() }}" alt="" />
                </a>
                <div class="p-5">
                    <a href="#">
                        <h5 class="mb-2 text-[20px] font-bold tracking-tight text-gray-900 dark:text-white">{{ $item->nama_surat }}</h5>
                    </a>
                    <p class="mb-3  font-normal text-gray-700 dark:text-gray-400">{{ $item->keterangan }}</p>
                    <a href="{{ $item->downloadSurat() }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center border text-ungu bg-white border-ungu rounded-lg hover:bg-ungu hover:text-white focus:ring-4 focus:outline-none focus:ring-purple-300 dark:bg-purple-600 dark:text-white dark:hover:bg-purple-700 dark:focus:ring-purple-800">
                        Gunakan Layanan
